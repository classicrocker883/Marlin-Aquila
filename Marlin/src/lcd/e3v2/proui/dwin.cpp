...
inline bool HMI_IsChinese()
{
  return HMI_flag.language == DWIN_CHINESE;
}
...
void ICON_AdvSettings()
{
  ...
  ICON_Button(select_page.now == PAGE_ADVANCE, ICON_Leveling_0, ico, txt, GET_TEXT_F(MSG_BUTTON_LEVEL));
  #else
...
void Popup_window_PauseOrStop()
{
  if (HMI_IsChinese())
  {
    ...
    if (select_print.now == PRINT_PAUSE_RESUME)
      DWIN_Frame_AreaCopy(1, 237, 338, 269, 356, 98, 150);
    else if (select_print.now == PRINT_STOP)
      DWIN_Frame_AreaCopy(1, 221, 320, 253, 336, 98, 150);
...
void ICON_ResumeOrPause()
{
  if (checkkey == PrintProcess)
    printingIsPaused() ? ICON_Resume() : ICON_Pause();
}
...
#if HAS_HOTEND
    if (_new_hotend_target)
      HotendTargetItem->redraw();
#endif
#if HAS_HEATED_BED
    if (_new_bed_target)
      BedTargetItem->redraw();
#endif
#if HAS_FAN
    if (_new_fanspeed)
      FanSpeedItem->redraw();
...
void make_name_without_ext(char *dst, char *src, size_t maxlen = MENU_CHAR_LIMIT)
{
  ...
  if (!pos)
    pos = strlen(src); // pos = 0 ('.' not found) restore pos
...
void Draw_SDItem(const uint16_t item, int16_t row = -1)
{
  ...
  const uint8_t icon = card.flag.filenameIsDir ? ICON_Folder : card.fileIsBinary() ? ICON_Binary
                                                                                   : ICON_File;
...
void DWIN_Draw_Dashboard()
{
  ...
  DWIN_Draw_Rectangle(1, HMI_data.SplitLine_Color, 0, 449, DWIN_WIDTH, 450);
...
void onClick_PauseOrStop()
{
  ...
  case PRINT_STOP:
    if (HMI_flag.select_flag)
      HMI_flag.abort_flag = true;
    break; // stop confirmed then abort print
  default:
    break;
  }
...
void HMI_Printing()
{
  ...
  else if (encoder_diffState == ENCODER_DIFF_ENTER)
  {
    ...
    case PRINT_PAUSE_RESUME:
      if (printingIsPaused())
      { // if printer is already in pause
        ui.resume_print();
        break;
      }
      else
...
void EachMomentUpdate()
{
  ...
  { // print process
    ...
    if (HMI_flag.abort_flag && !HMI_flag.home_flag)
    { // Print Stop
...
bool IDisPopUp()
{ // If ID is popup...
  return (checkkey == NothingToDo) || (checkkey == WaitResponse) || (checkkey == Homing) || (checkkey == Leveling) || (checkkey == PidProcess) || TERN0(HAS_ESDIAG, (checkkey == ESDiagProcess)) || (checkkey == Popup);
}
...
void HMI_SaveProcessID(const uint8_t id)
{
  ...
  {
    if (!IDisPopUp())
      last_checkkey = checkkey; // if previous is not a popup
    if ((id == Popup) || TERN0(HAS_ESDIAG, (id == ESDiagProcess)) || (id == PrintDone) || (id == Leveling) || (id == WaitResponse))
      wait_for_user = true;
    checkkey = id;
  }
}
...
{
  if (HMI_flag.abort_flag || checkkey == PrintDone)
    return;
...
void MarlinUI::pause_show_message(const PauseMessage message, const PauseMode mode /*=PAUSE_MODE_SAME*/, const uint8_t extruder /*=active_extruder*/)
{
  ...
  case PAUSE_MESSAGE_PARKING:
    DWIN_Popup_Pause(GET_TEXT_F(MSG_PAUSE_PRINT_PARKING));
    break; // M125
  case PAUSE_MESSAGE_CHANGING:
    DWIN_Popup_Pause(GET_TEXT_F(MSG_FILAMENT_CHANGE_INIT));
    break; // pause_print (M125, M600)
  case PAUSE_MESSAGE_WAITING:
    DWIN_Popup_Pause(GET_TEXT_F(MSG_ADVANCED_PAUSE_WAITING), BTN_Continue);
    break;
  case PAUSE_MESSAGE_INSERT:
    DWIN_Popup_Pause(GET_TEXT_F(MSG_FILAMENT_CHANGE_INSERT), BTN_Continue);
    break;
  case PAUSE_MESSAGE_LOAD:
    DWIN_Popup_Pause(GET_TEXT_F(MSG_FILAMENT_CHANGE_LOAD));
    break;
  case PAUSE_MESSAGE_UNLOAD:
    DWIN_Popup_Pause(GET_TEXT_F(MSG_FILAMENT_CHANGE_UNLOAD));
    break; // Unload of pause and Unload of M702
  ...
  case PAUSE_MESSAGE_RESUME:
    DWIN_Popup_Pause(GET_TEXT_F(MSG_FILAMENT_CHANGE_RESUME));
    break;
  case PAUSE_MESSAGE_HEAT:
    DWIN_Popup_Pause(GET_TEXT_F(MSG_FILAMENT_CHANGE_HEAT), BTN_Continue);
    break;
  case PAUSE_MESSAGE_HEATING:
    DWIN_Popup_Pause(GET_TEXT_F(MSG_FILAMENT_CHANGE_HEATING));
    break;
  case PAUSE_MESSAGE_STATUS:
    HMI_ReturnScreen();
    break; // Exit from Pause, Load and Unload
  default:
    break;
  }
}
...
void SaveMesh()
{
  TERN(AUTO_BED_LEVELING_UBL, UBLSaveMesh(), WriteEeprom());
}
#endif
#endif
...
void AutoLev()
{ // Always reacquire the Z "home" position
...
void LiveZOffset()
{
  ...
  if (BABYSTEP_ALLOWED())
    babystep.add_steps(Z_AXIS, step_zoffset);
#endif
}
...
void SetMoveX()
{
  if (!HMI_data.SetLiveMove)
  {
    ...
    SetPFloatOnClick(X_MIN_POS, X_MAX_POS, UNITFDIGITS, LiveMove);
  }
  else
  {
    HMI_value.axis = X_AXIS;
    SetPFloatOnClick(X_MIN_POS, X_MAX_POS, UNITFDIGITS, planner.synchronize, LiveMove);
  }
}
void SetMoveY()
{
  if (!HMI_data.SetLiveMove)
  {
    ...
    SetPFloatOnClick(Y_MIN_POS, Y_MAX_POS, UNITFDIGITS, LiveMove);
  }
  else
  {
    HMI_value.axis = Y_AXIS;
    SetPFloatOnClick(Y_MIN_POS, Y_MAX_POS, UNITFDIGITS, planner.synchronize, LiveMove);
  }
}
void SetMoveZ()
{
  if (!HMI_data.SetLiveMove)
  {
    HMI_value.axis = Z_AXIS;
    SetPFloatOnClick(Z_MIN_POS, Z_MAX_POS, UNITFDIGITS, LiveMove, planner.synchronize);
  }
  else
  {
    HMI_value.axis = Z_AXIS;
    SetPFloatOnClick(Z_MIN_POS, Z_MAX_POS, UNITFDIGITS, planner.synchronize, LiveMove);
  }
}
...
void HotendPID()
{
  SetPID(HMI_data.HotendPidT, H_E0);
}
#endif
#if HAS_HEATED_BED
void BedPID()
{
  SetPID(HMI_data.BedPidT, H_BED);
}
#endif
...
void SetBaud115K()
{
  queue.inject(F("M575 P0 B115200"));
  HMI_data.Baud250K = false;
}
void SetBaud250K()
{
  queue.inject(F("M575 P0 B250000"));
  HMI_data.Baud250K = true;
}
#endif
...
{
  ui.set_brightness(MenuData.Value);
}
...
#if ENABLED(CASELIGHT_USES_BRIGHTNESS)
void LiveCaseLightBrightness()
{
  caselight.brightness = MenuData.Value;
...
void LiveLEDColor(uint8_t *color)
{
  *color = MenuData.Value;
  leds.update();
}
...
#if HAS_HOME_OFFSET
void ApplyHomeOffset()
{
  set_home_offset(HMI_value.axis, MenuData.Value / MINUNITMULT);
}
void SetHomeOffsetX()
{
  HMI_value.axis = X_AXIS;
  SetPFloatOnClick(-50, 50, UNITFDIGITS, ApplyHomeOffset);
}
void SetHomeOffsetY()
{
  HMI_value.axis = Y_AXIS;
...
{
  HMI_value.axis = Z_AXIS;
  SetPFloatOnClick(-2, 2, UNITFDIGITS, ApplyHomeOffset);
}
#endif

#if HAS_BED_PROBE
void SetProbeOffsetX()
{
  SetPFloatOnClick(-60, 60, UNITFDIGITS);
}
...
void ApplyRunoutDistance()
{
  runout.set_runout_distance(MenuData.Value / MINUNITMULT);
}
...
void SetFilLoad()
{
  SetPFloatOnClick(0, MAX_LOAD_UNLOAD, UNITFDIGITS);
}
...
void ApplyExtMinT()
{
  thermalManager.extrude_min_temp = HMI_data.ExtMinT;
  thermalManager.allow_cold_extrude = (HMI_data.ExtMinT == 0);
}
...
#if HAS_HOTEND
void ApplyHotendTemp()
{
  thermalManager.setTargetHotend(MenuData.Value, 0);
}
...
void ApplyBedTemp()
{
  thermalManager.setTargetBed(MenuData.Value);
}
...
void Trammingwizard()
{
  ...
  float avg = 0.0f;
  LOOP_L_N(x, 2)
  LOOP_L_N(y, 2) avg += zval[x][y];
  avg /= 4.0f;
  LOOP_L_N(x, 2)
  LOOP_L_N(y, 2) zval[x][y] -= avg;
  MeshViewer.DrawMesh(zval, 2, 2);
  ui.reset_status();
  ...
  else
  {
    ...
    {
    case 0b00:
      plabel = GET_TEXT_F(MSG_LEVBED_FL);
      break;
    case 0b01:
      plabel = GET_TEXT_F(MSG_LEVBED_FR);
      break;
    case 0b10:
      plabel = GET_TEXT_F(MSG_LEVBED_BL);
      break;
    case 0b11:
      plabel = GET_TEXT_F(MSG_LEVBED_BR);
      break;
    default:
      plabel = F("");
      break;
...
#if HAS_PREHEAT
#if HAS_HOTEND
void SetPreheatEndTemp()
{
  SetPIntOnClick(MIN_ETEMP, MAX_ETEMP);
}
#endif
#if HAS_HEATED_BED
void SetPreheatBedTemp()
{
  SetPIntOnClick(MIN_BEDTEMP, MAX_BEDTEMP);
}
#endif
#if HAS_FAN
void SetPreheatFanSpeed()
{
  SetPIntOnClick(0, 255);
}
#endif
#endif
...
void ApplyMaxSpeed()
{
  planner.set_max_feedrate(HMI_value.axis, MenuData.Value / MINUNITMULT);
}
...
void SetMaxSpeedE()
{
  HMI_value.axis = E_AXIS;
  SetFloatOnClick(MIN_MAXFEEDSPEED, max_feedrate_edit_values[E_AXIS], UNITFDIGITS, planner.settings.max_feedrate_mm_s[E_AXIS], ApplyMaxSpeed);
}
#endif

void ApplyMaxAccel()
{
  planner.set_max_acceleration(HMI_value.axis, MenuData.Value);
}
...
{
  HMI_value.axis = E_AXIS;
  SetIntOnClick(MIN_MAXACCELERATION, max_acceleration_edit_values[E_AXIS], planner.settings.max_acceleration_mm_per_s2[E_AXIS], ApplyMaxAccel);
}
#endif
...
void ApplyMaxJerk()
{
  planner.set_max_jerk(HMI_value.axis, MenuData.Value / MINUNITMULT);
}
...
{
  HMI_value.axis = E_AXIS;
  SetFloatOnClick(MIN_MAXJERK, max_jerk_edit_values[E_AXIS], UNITFDIGITS, planner.max_jerk[E_AXIS], ApplyMaxJerk);
}
#endif
#endif
...
void SetLA_K()
{
  SetPFloatOnClick(0, 10, 3);
}
#endif

void SetStepsX()
{
  HMI_value.axis = X_AXIS, SetPFloatOnClick(MIN_STEP, MAX_STEP, UNITFDIGITS);
}
...
{
  HMI_value.axis = E_AXIS;
  SetPFloatOnClick(MIN_STEP, MAX_STEP, UNITFDIGITS);
}
#if ENABLED(PIDTEMP)
void SetHotendPidT()
{
...
#if ENABLED(FWRETRACT)
void Return_FWRetract_Menu()
{
  (PreviousMenu == FilamentMenu) ? Draw_FilamentMan_Menu() : Draw_Tune_Menu();
}
...
{
  if (HMI_IsChinese())
    menuitem->SetFrame(1, 129, 72, 156, 84);
...
{
  if (HMI_IsChinese())
    menuitem->SetFrame(1, 57, 104, 84, 116);
...
{
  if (HMI_IsChinese())
    menuitem->SetFrame(1, 87, 104, 114, 116);
  onDrawSubMenu(menuitem, line);
}
...
{
  if (HMI_IsChinese())
    menuitem->SetFrame(1, 117, 104, 172, 116);
  onDrawMenuItem(menuitem, line);
}
...
{
  if (HMI_IsChinese())
    menuitem->SetFrame(1, 174, 103, 229, 116);
  onDrawMenuItem(menuitem, line);
}
...
{
  if (HMI_IsChinese())
    menuitem->SetFrame(1, 1, 118, 56, 131);
  onDrawMenuItem(menuitem, line);
}
#endif
...
{
  if (HMI_IsChinese())
    menuitem->SetFrame(1, 231, 104, 258, 116);
  onDrawSubMenu(menuitem, line);
}
...
{
  if (HMI_IsChinese())
    menuitem->SetFrame(1, 58, 118, 106, 132);
...
{
  if (HMI_IsChinese())
    menuitem->SetFrame(1, 109, 118, 157, 132);
  onDrawPFloatMenu(menuitem, line);
}
...
{
  if (HMI_IsChinese())
    menuitem->SetFrame(1, 160, 118, 209, 132);
  onDrawPFloatMenu(menuitem, line);
}
...
{
  if (HMI_IsChinese())
    menuitem->SetFrame(1, 212, 118, 253, 131);
  onDrawPFloatMenu(menuitem, line);
}
#endif
...
{
  if (HMI_IsChinese())
    menuitem->SetFrame(1, 159, 70, 200, 84);
  onDrawSubMenu(menuitem, line);
}
...
{
  if (HMI_IsChinese())
    menuitem->SetFrame(1, 204, 70, 259, 82);
  onDrawMenuItem(menuitem, line);
}
...
{
  if (HMI_IsChinese())
    menuitem->SetFrame(1, 0, 89, 41, 101);
  onDrawMenuItem(menuitem, line);
}
...
{
  if (HMI_IsChinese())
    menuitem->SetFrame(1, 174, 164, 223, 177);
...
{
  if (HMI_IsChinese())
    menuitem->SetFrame(1, 43, 89, 98, 101);
  onDrawMenuItem(menuitem, line);
}
#endif
#endif
...
{
  if (HMI_IsChinese())
    menuitem->SetFrame(1, 100, 89, 151, 101);
  onDrawMenuItem(menuitem, line);
}
#if PREHEAT_COUNT > 1
...
{
  if (HMI_IsChinese())
    menuitem->SetFrame(1, 180, 89, 233, 100);
  onDrawMenuItem(menuitem, line);
}
#endif
#endif
...
{
  if (HMI_IsChinese())
    menuitem->SetFrame(1, 1, 104, 56, 117);
  onDrawMenuItem(menuitem, line);
}
#endif
...
{
  if (HMI_IsChinese())
    menuitem->SetFrame(1, 239, 134, 266, 146);
  onDrawMenuItem(menuitem, line);
...
{
  if (HMI_IsChinese())
    menuitem->SetFrame(1, 116, 164, 171, 176);
...
{
  if (HMI_IsChinese())
    menuitem->SetFrame(1, 1, 134, 56, 146);
  onDrawPIntMenu(menuitem, line);
}
#endif
...
{
  if (HMI_IsChinese())
    menuitem->SetFrame(1, 58, 134, 113, 146);
  onDrawPIntMenu(menuitem, line);
}
#endif
...
{
  if (HMI_IsChinese())
    menuitem->SetFrame(1, 115, 134, 170, 146);
...
{
  if (HMI_IsChinese())
    menuitem->SetFrame(1, 100, 89, 178, 101);
  onDrawSubMenu(menuitem, line);
}
#if PREHEAT_COUNT >= 2
...
{
  if (HMI_IsChinese())
    menuitem->SetFrame(1, 180, 89, 260, 100);
  onDrawSubMenu(menuitem, line);
}
#endif
#endif // HAS_PREHEAT
...
#if HAS_ONESTEP_LEVELING
void onDrawManualTramming(MenuItemClass *menuitem, int8_t line)
{
  onDrawChkbMenu(menuitem, line, HMI_data.FullManualTramming);
}
#endif
...
#if PREHEAT_COUNT > 1
    MENU_ITEM_F(ICON_ABSPreheat, MSG_PREHEAT_2, onDrawPreheat2, DoPreheat1);
#if PREHEAT_COUNT > 2
    MENU_ITEM_F(ICON_CustomPreheat, MSG_PREHEAT_3, onDrawMenuItem, DoPreheat2);
#endif
#endif
#endif
...
#endif
#if ENABLED(BAUD_RATE_GCODE)
    EDIT_ITEM(ICON_SetBaudRate, F("250K baud"), onDrawChkbMenu, SetBaudRate, &HMI_data.Baud250K);
#endif
    MENU_ITEM_F(ICON_Scolor, MSG_COLORS_SELECT, onDrawSubMenu, Draw_SelectColors_Menu);
#if ENABLED(SOUND_MENU_ITEM)
    EDIT_ITEM_F(ICON_Sound, MSG_TICK, onDrawChkbMenu, SetEnableTick, &ui.no_tick); //changed
    EDIT_ITEM_F(ICON_Sound, MSG_SOUND, onDrawChkbMenu, SetEnableSound, &ui.sound_on);
#endif
#if HAS_ESDIAG
...
{
  checkkey = Menu;
  if (SetMenu(AdvancedSettings, GET_TEXT_F(MSG_UBL_LEVEL_BED), 10))
  {
    BACK_ITEM(Goto_Main_Menu);
#if ENABLED(EEPROM_SETTINGS)
...
#endif
    EDIT_ITEM(ICON_Version, F("Live Adjustment"), onDrawChkbMenu, SetLiveMove, &HMI_data.SetLiveMove);
  }
  CurrentMenu->draw();
  if (!all_axes_trusted())
    LCD_MESSAGE_F("WARNING: Current position unknown. Home axes.");
}
...
{
  checkkey = Menu;
  if (SetMenu(FilSetMenu, GET_TEXT_F(MSG_FILAMENT_SET), 8))
  {
...
{
  checkkey = Menu;
  if (SetMenu(SelectColorMenu, GET_TEXT_F(MSG_COLORS_SELECT), 21))
  {
    ...
    EDIT_ITEM(0, F("Button Color"), onDrawSelColorItem, SelColor, &HMI_data.Button_Color);
  }
...
#endif
#if ENABLED(LIN_ADVANCE)
    EDIT_ITEM_F(ICON_MaxAccelerated, MSG_ADVANCE_K, onDrawPFloat3Menu, SetLA_K, &planner.extruder_advance_K[0]);
#endif
#if HAS_LCD_BRIGHTNESS
    EDIT_ITEM_F(ICON_Brightness, MSG_BRIGHTNESS, onDrawPInt8Menu, SetBrightness, &ui.brightness);
#endif
...
{
  checkkey = Menu;
  if (SetMenu(FilamentMenu, GET_TEXT_F(MSG_FILAMENT_MAN), 6))
  {
    BACK_ITEM(Draw_Prepare_Menu);
    MENU_ITEM_F(ICON_FilMan, MSG_FILAMENTCHANGE, onDrawMenuItem, ChangeFilament);
#if ENABLED(FILAMENT_LOAD_UNLOAD_GCODES)
...
void Draw_Preheat1_Menu()
{
  HMI_value.Select = 0;
  if (!PreheatMenu)
    PreheatMenu = new MenuClass();
  Draw_Preheat_Menu({59, 16, 81, 14}, GET_TEXT_F(MSG_PREHEAT_1_SETTINGS)); // TODO: English "PLA Settings" JPG
}
...
void Draw_Preheat2_Menu()
{
  ...
  if (!PreheatMenu)
    PreheatMenu = new MenuClass();
  Draw_Preheat_Menu({142, 16, 82, 14}, GET_TEXT_F(MSG_PREHEAT_2_SETTINGS)); // TODO: English "ABS Settings" JPG
}
#endif
...
void Draw_Preheat3_Menu()
{
  ...
  if (!PreheatMenu)
    PreheatMenu = new MenuClass();
  Draw_Preheat_Menu({225, 16, 82, 14}, GET_TEXT_F(MSG_PREHEAT_3_SETTINGS)); // TODO: English "... Settings" JPG
}
#endif
...
void Draw_Temperature_Menu()
{
  ...
  if (CurrentMenu != TemperatureMenu)
  {
    ...
    MENU_ITEM(ICON_Temperature, F("PID Settings"), onDrawSubMenu, Draw_PID_Menu);
#endif
#if HAS_PREHEAT
...
{
  checkkey = Menu;
  if (SetMenu(PIDMenu, F("PID Settings"), 3))
  {
    BACK_ITEM(Draw_Temperature_Menu);
#ifdef PIDTEMP
...
void Draw_HotendPID_Menu()
{
  ...
  if (SetMenu(HotendPIDMenu, F(STR_HOTEND_PID " Settings"), 8))
  {
    ...
    MENU_ITEM(ICON_PIDNozzle, F("Tune " STR_HOTEND_PID), onDrawMenuItem, HotendPID);
...
void Draw_BedPID_Menu()
{
  ...
  {
    BACK_ITEM(Draw_PID_Menu);
    MENU_ITEM(ICON_PIDNozzle, F("Tune " STR_BED_PID), onDrawMenuItem, BedPID);
...
{
  checkkey = Menu;
  if (SetMenu(ZOffsetWizMenu, GET_TEXT_F(MSG_PROBE_WIZARD), 6))
  {
    BACK_ITEM(Draw_Prepare_Menu);
    MENU_ITEM_F(ICON_Homing, MSG_AUTO_HOME, onDrawMenuItem, AutoHome);
    MENU_ITEM_F(ICON_MoveZ0, MSG_MOVE_NOZZLE_TO_BED, onDrawMenuItem, SetMoveZto0);
    EDIT_ITEM_F(ICON_Fade, MSG_XATC_UPDATE_Z_OFFSET, onDrawPFloat2Menu, SetZOffset, &BABY_Z_VAR);
    MENU_ITEM(0, "For Best Results:\n", onDrawMenuItem);
    MENU_ITEM(ICON_HotendTemp, "Make Nozzle Touch Bed", onDrawMenuItem);
  }
  UpdateMenu(ZOffsetWizMenu);
  if (!axis_is_trusted(Z_AXIS))
    LCD_MESSAGE_F("WARNING: unknown Z position, Home Z axis...");
}
#endif
...
{
  checkkey = Menu;
  if (SetMenu(HomingMenu, GET_TEXT_F(MSG_HOMING), 6))
  {
    BACK_ITEM(Draw_Prepare_Menu);
    MENU_ITEM_F(ICON_Homing, MSG_AUTO_HOME, onDrawMenuItem, AutoHome);
...
void Draw_FWRetract_Menu()
{
  ...
  if (SetMenu(FWRetractMenu, GET_TEXT_F(MSG_FWRETRACT), 6))
  {
    ...
    EDIT_ITEM_F(ICON_FWRecSpeed, MSG_SINGLENOZZLE_UNRETRACT_SPEED, onDrawPFloatMenu, SetRetractSpeed, &fwretract.settings.retract_recover_feedrate_mm_s);
...
#define Z_OFFSET_MAX 3
    ...
    void SetAutoMovToMesh() {
      ...
      Draw_Chkb_Line(CurrentMenu->line(), AutoMovToMesh);
      DWIN_UpdateLCD(); 
      }
#endif
#endif
...
void ApplyUBLSlot()
{
  bedlevel.storage_slot = MenuData.Value;
}
...
{
  checkkey = Menu;
  if (SetMenu(MeshMenu, GET_TEXT_F(MSG_MESH_LEVELING), 9))
  {
    BACK_ITEM(Draw_AdvancedSettings_Menu);
#if ENABLED(AUTO_BED_LEVELING_UBL)
    EDIT_ITEM_F(ICON_SetZOffset, MSG_Z_FADE_HEIGHT, onDrawPFloatMenu, SetMeshFadeHeight, &planner.z_fade_height);
    EDIT_ITEM_F(ICON_Version, MSG_ACTIVATE_MESH, onDrawChkbMenu, SetMeshActive, &planner.leveling_active);
    EDIT_ITEM_F(ICON_ResumeEEPROM, MSG_UBL_STORAGE_SLOT, onDrawUBLSlot, SetUBLSlot, &bedlevel.storage_slot);
    MENU_ITEM_F(ICON_WriteEEPROM, MSG_UBL_SAVE_MESH, onDrawMenuItem, UBLSaveMesh);
    MENU_ITEM_F(ICON_ReadEEPROM, MSG_UBL_LOAD_MESH, onDrawMenuItem, UBLLoadMesh);
    EDIT_ITEM_F(ICON_ProbeMargin, MSG_UBL_TILTING_GRID, onDrawPInt8Menu, SetUBLTiltGrid, &BedLevelTools.tilt_grid);
    MENU_ITEM_F(ICON_HSMode, MSG_UBL_SMART_FILLIN, onDrawMenuItem, UBLSmartFillMesh);
#endif
  }
  UpdateMenu(MeshMenu);
}
...
void Draw_EditMesh_Menu()
{
  ...
  checkkey = Menu;
  if (SetMenu(EditMeshMenu, GET_TEXT_F(MSG_EDIT_MESH), 6))
  {
    BedLevelTools.mesh_x = BedLevelTools.mesh_y = 0;
    BACK_ITEM(Draw_AdvancedSettings_Menu);
    EDIT_ITEM_F(ICON_SetHome, MSG_PROBE_WIZARD_MOVING, onDrawChkbMenu, SetAutoMovToMesh, &AutoMovToMesh);
    EDIT_ITEM_F(ICON_HomeX, MSG_MESH_X, onDrawPInt8Menu, SetEditMeshX, &BedLevelTools.mesh_x);
    EDIT_ITEM_F(ICON_HomeY, MSG_MESH_Y, onDrawPInt8Menu, SetEditMeshY, &BedLevelTools.mesh_y);
    EditZValueItem = EDIT_ITEM_F(ICON_HomeZ, MSG_MESH_EDIT_Z, onDrawPFloat3Menu, SetEditZValue, &bedlevel.z_values[BedLevelTools.mesh_x][BedLevelTools.mesh_y]);
    TERN_(HAS_BED_PROBE, MENU_ITEM_F(ICON_HSMode, MSG_PROBE_WIZARD_PROBING, onDrawMenuItem, BedLevelTools.ProbeXY));
  }
...
{
  checkkey = Menu;
  if (SetMenu(ManualMesh, GET_TEXT_F(MSG_UBL_MANUAL_MESH), 8))
  {
    BACK_ITEM(Draw_AdvancedSettings_Menu);
    EDIT_ITEM_F(ICON_SetZOffset, MSG_Z_FADE_HEIGHT, onDrawPFloatMenu, SetMeshFadeHeight, &planner.z_fade_height);
    EDIT_ITEM_F(ICON_Version, MSG_ACTIVATE_MESH, onDrawChkbMenu, SetMeshActive, &planner.leveling_active);
